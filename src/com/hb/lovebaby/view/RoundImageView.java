...
//       int radius1 = (int) (centre - mBorderThickness/2); //圆环的半径
//       paint.setColor(color);//(roundColor); //设置圆环的颜色
//       paint.setStyle(Paint.Style.STROKE); //设置空心
//       paint.setStrokeWidth(mBorderThickness); //设置圆环的宽度
//       paint.setAntiAlias(true);  //消除锯齿
//       canvas.drawCircle(centre, centre, radius1, paint); //画出圆环

        startProgressBar(canvas);
    }
    
    private void startProgressBar(Canvas canvas){
...
//       paint.setColor(Color.RED);//设置进度的颜色
       LinearGradient lg=new LinearGradient(0,0,100,100,Color.GREEN,Color.YELLOW,Shader.TileMode.MIRROR);
       paint.setShader(lg);
       final RectF oval = new RectF(centre - radius1, centre - radius1, centre
                + radius1, centre + radius1);  //用于定义的圆弧的形状和大小的界限

       paint.setStyle(Paint.Style.STROKE);
       canvas.drawArc(oval, start, 360 * progress / 100, false, paint);  //根据进度画圆弧  绘制白色圆圈，即进度条背景
    }
